    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            if response.status_code == 429:
                ...
                sys.stderr.write(f"Nginx control cooldown (429), retry {attempt}/{MAX_RETRIES} in {wait:.1f}s\n")
                time.sleep(wait)
                continue
            ...
            if attempt < MAX_RETRIES:
                wait = BACKOFF_BASE * (2 ** (attempt - 1))
                sys.stderr.write(f"Nginx control call failed (attempt {attempt}/{MAX_RETRIES}): {exc}, retrying in {wait:.1f}s\n")
                time.sleep(wait)
            else:
                sys.stderr.write(f"Nginx control call failed after {MAX_RETRIES} attempts: {last_exc}\n")
    return 1
